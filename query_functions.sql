...
CREATE FUNCTION make_budget (picked_genre VARCHAR(20))
	RETURNS BIGINT as $$
	DECLARE new_budget BIGINT;
	BEGIN
...
CREATE FUNCTION cost_of_hire (id INT, dir INT) --0 if actor, 1 if director
	RETURNS BIGINT as $$
	DECLARE cost BIGINT;
	BEGIN
...
CREATE FUNCTION rating_generated_m (id INT, dir INT)
...
CREATE FUNCTION revenue (id INT, dir INT) --0 if actor, 1 if director
	RETURNS BIGINT as $$
	DECLARE makes BIGINT;
	BEGIN
